...
export function Login() {
  ...
  const loginWithGoogle = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      try {
        ...
        useAuthStore.getState().login({ name: userInfo.given_name || 'Google User', email: userInfo.email });
        navigate('/analyze');
      } catch (err) {
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    */
    ...
    // Dummy login logic (Remove when backend is ready)
    useAuthStore.getState().login({ name: email.split('@')[0], email });
    navigate('/analyze');
  };
    ...
    <main className="flex-grow flex items-center justify-center p-4 md:p-8 relative overflow-hidden">
      ...
      >
        <div className="text-center mb-8">
          <h1 className="font-display-lg-mobile text-on-surface mb-2 tracking-tight">Welcome Back</h1>
          <p className="text-on-surface-variant font-body-md">Sign in to continue to Marginal.</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <label className="block text-sm font-label-mono text-on-surface-variant uppercase tracking-wider">
                Password
              </label>
              <a href="#" className="text-sm font-body-md text-primary hover:underline">Forgot?</a>
            </div>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-on-surface-variant/50">
                <Lock size={18} />
